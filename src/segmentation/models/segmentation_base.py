...
class BaseSegmentationModel(BaseModel):
    ...
    class _Schema(Schema):
        n_classes = fields.Int(required=True, metadata={'description': 'Number of classes', 'example': 2})
        learning_rate = fields.Float(load_default=None, metadata={'description': 'Learning rate used in training.', 'example': 0.01})
        loss = fields.String(load_default='cross_entropy', metadata={'description': 'Loss function used for training.'},
                             validate=OneOf(segmentation_losses.keys()))
        metrics = fields.List(fields.String, load_default=['accuracy', 'iou'],
                              metadata={'description': 'List of metrics used for evaluation.'},
                              validate=ContainsOnly(segmentation_metrics.keys()))

        class_weights = fields.Dict(load_default=None, metadata={'description': 'Dictionary mapping class id with weight. '
                                                              'If key for some labels is not specified, 1 is used.'})

        prediction_visualization = fields.Bool(load_default=False, metadata={'description': 'Record prediction visualization summaries.'})
        prediction_visualization_num = fields.Int(load_default=5,
                                                  metadata={'description': 'Number of images used for prediction visualization.'})
