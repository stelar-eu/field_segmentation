...
class FCNModel(BaseSegmentationModel):
    ...
    class FCNModelSchema(BaseSegmentationModel._Schema):
        n_layers = fields.Int(required=True, metadata={'description': 'Number of layers of the FCN model', 'example': 10})
        keep_prob = fields.Float(required=True, metadata={'description': 'Keep probability used in dropout layers.', 'example': 0.5})
        features_root = fields.Int(required=True, metadata={'description': 'Number of features at the root level.', 'example': 32})

        conv_size = fields.Int(load_default=3, metadata={'description': 'Size of the convolution kernels.'})
        deconv_size = fields.Int(load_default=2, metadata={'description': 'Size of the deconvolution kernels.'})
        conv_stride = fields.Int(load_default=1, metadata={'description': 'Stride used in convolutions.'})
        dilation_rate = fields.List(fields.Int, load_default=1, metadata={'description': 'Dilation rate used in convolutions.'})
        add_dropout = fields.Bool(load_default=False, metadata={'description': 'Add dropout to layers.'})
        add_batch_norm = fields.Bool(load_default=True, metadata={'description': 'Add batch normalization to layers.'})
        bias_init = fields.Float(load_default=0.0, metadata={'description': 'Bias initialization value.'})
        use_bias = fields.Bool(load_default=True, metadata={'description': 'Add bias parameters to convolutional layer.'})
        padding = fields.String(load_default='VALID', metadata={'description': 'Padding type used in convolutions.'})

        pool_size = fields.Int(load_default=2, metadata={'description': 'Kernel size used in max pooling.'})
        pool_stride = fields.Int(load_default=2, metadata={'description': 'Stride used in max pooling.'})

        class_weights = fields.List(fields.Float, load_default=None, metadata={'description': 'Class weights used in training.'})
...
class TFCNModel(BaseSegmentationModel):
    ...
    class TFCNModelSchema(BaseSegmentationModel._Schema):
        ...
        conv_size = fields.Int(load_default=3, metadata={'description':'Size of the convolution kernels.'})
        deconv_size = fields.Int(load_default=2, metadata={'description':'Size of the deconvolution kernels.'})
        conv_size_reduce = fields.Int(load_default=3, metadata={'description': 'Size of the kernel for time reduction.'})
        conv_stride = fields.Int(load_default=1, metadata={'description': 'Stride used in convolutions.'})
        add_dropout = fields.Bool(load_default=False, metadata={'description': 'Add dropout to layers.'})
        add_batch_norm = fields.Bool(load_default=True, metadata={'description': 'Add batch normalization to layers.'})
        bias_init = fields.Float(load_default=0.0, metadata={'description': 'Bias initialization value.'})
        use_bias = fields.Bool(load_default=True, metadata={'description': 'Add bias parameters to convolutional layer.'})
        padding = fields.String(load_default='VALID', metadata={'description': 'Padding type used in convolutions.'})
        single_encoding_conv = fields.Bool(load_default=False, metadata={'description': 'Whether to apply 1 or 2 banks of conv filters.'})

        pool_size = fields.Int(load_default=2, metadata={'description': 'Kernel size used in max pooling.'})
        pool_stride = fields.Int(load_default=2, metadata={'description': 'Stride used in max pooling.'})
        pool_time = fields.Bool(load_default=False, metadata={'description': 'Operate pooling over time dimension.'})
