...
class TCNModel(BaseClassificationModel):
    ...
    class TCNModelSchema(BaseClassificationModel._Schema):
        keep_prob = fields.Float(required=True, metadata={'description': 'Keep probability used in dropout layers.', 'example': 0.5})

        kernel_size = fields.Int(load_default=2, metadata={'description': 'Size of the convolution kernels.'})
        nb_filters = fields.Int(load_default=64, metadata={'description': 'Number of convolutional filters.'})
        nb_stacks = fields.Int(load_default=1)
        dilations = fields.List(fields.Int, load_default=[1, 2, 4, 8, 16, 32], metadata={'description': 'Size of dilations used in the covolutional layers'})
        padding = fields.String(load_default='CAUSAL', validate=OneOf(['CAUSAL', 'SAME']),
                                metadata={'description': 'Padding type used in convolutions.'})
        use_skip_connections = fields.Bool(load_default=True, metadata={'description': 'Flag to whether to use skip connections.'})
        return_sequences = fields.Bool(load_default=False, metadata={'description': 'Flag to whether return sequences or not.'})
        activation = fields.Str(load_default='linear', metadata={'description': 'Activation function used in final filters.'})
        kernel_initializer = fields.Str(load_default='he_normal', metadata={'description': 'method to initialise kernel parameters.'})

        use_batch_norm = fields.Bool(load_default=False, metadata={'description': 'Whether to use batch normalisation.'})
        use_layer_norm = fields.Bool(load_default=False, metadata={'description': 'Whether to use layer normalisation.'})
...
class TempCNNModel(BaseClassificationModel):
    ...
    class TempCNNModelSchema(BaseClassificationModel._Schema):
        ...
        kernel_size = fields.Int(load_default=5, metadata={'description': 'Size of the convolution kernels.', 'example': 5})
        nb_conv_filters = fields.Int(load_default=16, metadata={'description': 'Number of convolutional filters.', 'example': 16})
        nb_conv_stacks = fields.Int(load_default=3, metadata={'description': 'Number of convolutional blocks.', 'example': 3})
        nb_conv_strides = fields.Int(load_default=1, metadata={'description': 'Value of convolutional strides.', 'example': 1})
        nb_fc_neurons = fields.Int(load_default=256, metadata={'description': 'Number of Fully Connect neurons.', 'example': 256})
        nb_fc_stacks = fields.Int(load_default=1, metadata={'description': 'Number of fully connected layers.', 'example': 1})
        padding = fields.String(load_default='SAME', validate=OneOf(['SAME']),
                                metadata={'description': 'Padding type used in convolutions.', 'example': 'SAME'})
        activation = fields.Str(load_default='relu', metadata={'description': 'Activation function used in final filters.', 'example': 'relu'})
        kernel_initializer = fields.Str(load_default='he_normal', metadata={'description': 'Method to initialise kernel parameters.'})
        kernel_regularizer = fields.Float(load_default=1e-6, metadata={'description': 'L2 regularization parameter.'})
...
class BiRNN(BaseClassificationModel):
    ...
    class BiRNNModelSchema(BaseClassificationModel._Schema):
        rnn_layer = fields.String(required=True, validate=OneOf(['rnn', 'lstm', 'gru']),
                                  metadata={'description': 'Type of RNN layer to use'})

        keep_prob = fields.Float(required=True, metadata={'description': 'Keep probability used in dropout layers.', 'example': 0.5})

        rnn_units = fields.Int(load_default=64, metadata={'description': 'Size of the convolution kernels.'})
        rnn_blocks = fields.Int(load_default=1, metadata={'description': 'Number of LSTM blocks'})
        bidirectional = fields.Bool(load_default=True, metadata={'description': 'Whether to use a bidirectional layer'})

        activation = fields.Str(load_default='linear', metadata={'description': 'Activation function used in final dense filters.'})
        kernel_initializer = fields.Str(load_default='he_normal', metadata={'description': 'Method to initialise kernel parameters.'})
        kernel_regularizer = fields.Float(load_default=1e-6, metadata={'description': 'L2 regularization parameter.'})

        layer_norm = fields.Bool(load_default=True, metadata={'description': 'Whether to apply layer normalization in the encoder.'})
        batch_norm = fields.Bool(load_default=False, metadata={'description': 'Whether to use batch normalisation.'})
...
class TransformerEncoder(BaseClassificationModel):
    ...
    class TransformerEncoderSchema(BaseClassificationModel._Schema):
        ...
        num_heads = fields.Int(load_default=8, metadata={'description': 'Number of Attention heads.'})
        num_layers = fields.Int(load_default=4, metadata={'description': 'Number of encoder layers.'})
        num_dff = fields.Int(load_default=512, metadata={'description': 'Number of feed-forward neurons in point-wise MLP.'})
        d_model = fields.Int(load_default=128, metadata={'description': 'Depth of model.'})
        max_pos_enc = fields.Int(load_default=24, metadata={'description': 'Maximum length of positional encoding.'})
        layer_norm = fields.Bool(load_default=True, metadata={'description': 'Whether to apply layer normalization in the encoder.'})
...
class PseTae(BaseClassificationModel):
    ...
    class PseTaeSchema(BaseClassificationModel._Schema):
        mlp1 = fields.List(fields.Int, load_default=[10, 32, 64], metadata={'description': 'Number of units for each layer in mlp1.'})
        pooling = fields.Str(load_default='mean_std', metadata={'description': 'Methods used for pooling. Seperated by underscore. (mean, std, max, min)'})
        mlp2 = fields.List(fields.Int, load_default=[132, 128], metadata={'description': 'Number of units for each layer in mlp2.'})

        num_heads = fields.Int(load_default=4, metadata={'description': 'Number of Attention heads.'})
        num_dff = fields.Int(load_default=32, metadata={'description': 'Number of feed-forward neurons in point-wise MLP.'})
        d_model = fields.Int(load_default=None, metadata={'description': 'Depth of model.'})
        mlp3 = fields.List(fields.Int, load_default=[512, 128, 128], metadata={'description': 'Number of units for each layer in mlp3.'})
        dropout = fields.Float(load_default=0.2, metadata={'description': 'Dropout rate for attention encoder.'})
        T = fields.Float(load_default=1000, metadata={'description': 'Number of features for attention.'})
        len_max_seq = fields.Int(load_default=24, metadata={'description': 'Number of features for attention.'})
        mlp4 = fields.List(fields.Int, load_default=[128, 64, 32], metadata={'description': 'Number of units for each layer in mlp4. Last layer with n_classes is added automatically.'})
